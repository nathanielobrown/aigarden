...
The validation gate for adopting aigarden in mycelia in place of its
hand-rolled `tools/` lint gates. It shadow-runs aigarden against mycelia's whole working tree,
...
below, aigarden's residual over mycelia was **193 findings from one unbuilt feature** (the
...
|---|---:|---:|---|
| 1 — aigarden miss (mycelia finds, aigarden doesn't) | 0 observable | 0 observable | mycelia's suite is green on this tree, so misses can't surface here (see Method) |
| 2 — aigarden extra / **win** (genuine, mycelia can't see) | 1 | 1 | `backend/scripts/…` length scope-gap — kept |
| 3 — aigarden false positive | 515 | 0 | ~101 grammar FPs fixed in code; 192 `code-doc-ref` + 6 `reports` suppressed by tuned config |
...
- **aigarden** — commit `1a1e2d0` (this repo, after the fixes below), `cargo build --release`,
  rumdl_lib `0.2.39`. Run as `aigarden check --config <tuned> --output-format json` from the mycelia
  root over the same 1274 files.

**The tuned config** translates mycelia's gate wiring into aigarden keys (excludes union +
dir-scoped length budgets with `use-defaults = false` + `markdown-style` off, since mycelia's rumdl
set — MD013 reflow + MD031 — differs from aigarden's MD009/010/012/047). Its full text is in the
handoff; it lives in `/tmp`, never committed to mycelia.

**Limitation — the green-tree blind spot.** mycelia's suite passing means category 1 (aigarden
...
aigarden finding mycelia doesn't emit is explained) and by construction (aigarden's reference core
...
- **`0877015` — bare-path grammar (category 3, ~101 FPs).** mycelia's `check_bare_paths` rejects a
  ...
  aigarden's `looks_like_path` now applies the same guards (`BAD_SPAN_CHARS`, `NNNN`, `.git/`), and
...
tuned config, **not** a code fix. At run time a faithful mapping needed *per-rule* scoping aigarden
...
- **Length coverage beyond mycelia's dir list.** mycelia gates `backend/mycelia backend/tests
  ...
  aigarden's generic default catches it. Mild — mycelia scoped this out on purpose — but a real
  scope gap surfaced by the run, and the kind of drift a generic default is meant to stop.
- **By construction, not counted above:** aigarden reports *all* findings in one pass (mycelia's
...
**None** — every gap this run named has shipped (see below). aigarden's residual over mycelia is now
...
- **Per-rule scoping (was Med, 192 findings masked by a workaround)** — shipped as
  ...
  workaround. aigarden dogfoods it — see this repo's `aigarden.toml`
...
wrapping ruff/pyrefly/prettier is [out of scope](roadmap.md#explicitly-out-of-scope). aigarden's cog
marker grammar (`aigarden:cog`/`aigarden:end`) is a clean break from mycelia's `[[[cogs]]]`, so cog
...
- **D3 — do empty default excludes hold up, or is `**/fixtures/**` needed? Needed — changed the
  ...
  hygiene tool in mycelia already excludes fixtures; aigarden now does too, out of the box, unioned
