...
rule id. A cog marker line counts as the blank line beside a fence, and a finding inside a \
generated cog block names the generator to fix (docs/design.md, \"Cogs and markdown style\").",
            config: &[ConfigKey {
                key: "reflow",
                default: "\"off\"",
...
ensures a single final newline (and reflows paragraphs when reflow names a mode). It never edits \
inside a cog block: the generator owns those bytes.",
            ),
            config_gated: false,
        }
    }
    fn check(&self, ctx: &RuleContext<'_>) -> Vec<Diagnostic> {
        ...
        for finding in &run(group, &style_rules(reflow)) {
            ...
            let layout = layouts
                .entry(&finding.file.rel_path)
                .or_insert_with(|| CogLayout::parse(&finding.file.content).ok());
            let owner = layout
                .as_ref()
                .map_or(Some(Owner::Author), |l| l.owner(&finding.warning));
            let Some(owner) = owner else {
                continue; // a cog marker supplies the blank line this rule wants
            };
            let raw = &finding.warning.message;
            let mut text = never_wrap_message(reflow, raw).unwrap_or_else(|| raw.clone());
            if let Owner::Generator(directive) = owner {
                text = format!(
                    "{text} \u{2014} generated by `{directive}`, so fix the generator \
(`--fix` leaves cog blocks alone)"
                );
            }
